...
+ (NSMutableArray *)loadData { //+ (Chapters *)loadData {
    ...
    for (GDataXMLElement *element in dataArray) {
        ...
        NSArray *iconArray = [element elementsForName:@"Icon"];
        ...
        if (iconArray.count > 0) {
            GDataXMLElement *iconElement = (GDataXMLElement *) [iconArray objectAtIndex:0];
            icon = [iconElement stringValue] ;
        }
        ...
        Chapter *chapter = [[Chapter alloc] initWithName:name icon:icon number:number];
